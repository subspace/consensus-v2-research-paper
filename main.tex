...
Aim: to show that the cost of a single lottery ticket is lowest if following the protocol (dedicating storage for plotting)

--> underlying assumption that all tickets are independent, so the analysis for a single ticket suffices.
...
Claim: let F be a farmer with storage S. Let $T_S$ be the token cost of following the farming protocol per challenge*. Let $T^*$ be the cost of obtaining tickets (define better) in a hybrid strategy that combines storage and compute. Then $T^* \geq T_S$.**
...
- firstly, we treat MASK (the plotting function) as a block box. That is, one cannot open it, but only enter inputs and recieve outputs. Hence, MASK has a fixed cost per invocation.
