...
Applying this procedure to all $\{ g_i^{\id}(\id + j) \}_{i = 0}^{m - 1}$, we obtain a {\bf masked storage coin}\footnote{Alternatively, 
we can set $\ind = j$ (where $j \in [2^k]$) and use $H\left( \mask(\seed, \ind; k) \right)$ to mask $g_i^{\id}(j)$ instead of $g_i^{\id}(\id + j)$. That is, the start index can be $0$
instead of $\id$, and the storage coins associated with $\id$ are at points $0$ to $\ell - 1$.}.
